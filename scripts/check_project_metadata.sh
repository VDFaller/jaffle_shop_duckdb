...
if [[ -n "${DBT_FUSION_BIN:-}" ]]; then
  fusion_dbt=("$DBT_FUSION_BIN")
elif command -v dbtf >/dev/null 2>&1; then
  fusion_dbt=(dbtf)
elif [[ -x "/home/fal81743/.local/bin/dbt" ]]; then
  fusion_dbt=(/home/fal81743/.local/bin/dbt)
else
  echo "Could not find dbt Fusion. Set DBT_FUSION_BIN or install dbtf." >&2
  exit 1
fi
